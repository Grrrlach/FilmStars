...
@main.route('/edit_review/<int:review_id>', methods = ['GET', 'PUT', "POST"])
@login_required
def edit_review(review_id):
    ...
    return render_template('edit_review.html.j2', self=review, form=form)
...
def write_review():
    ...
    # ###########  IMPORTED FROM MOVIE_SEARCH()  ################
        ...
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            flash("Sadly, something went wrong. Most likely your movie title or year was incorrect. Please try again", 'danger')
...
def search():
    ...
    if request.method == 'POST':
        ...
        movie_desc = primary_movie_data["overview"]
        ...
        # return "DUMBY"
        return render_template('search_page.html.j2', form = form, movie_dict=movie_dict)
    # return "DUMBY"
    return render_template('search_page.html.j2', form=form)
...
def my_reviews():
    ...
    try:
        ...
        flash ('You have no movie reviews', "warning")
        return render_template('write_review.html.j2')
...
def movies_to_watch():
    ...
    while True:
        try:
            ...
            g.movie_blurb = f"{random_movie}:  {random_movie_desc}"
            break
            # return movie_blurb
...
@main.route('/delete_review/<int:review_id>')
@login_required
def delete_review(review_id):
    # needs authentication of user == review author
    # until then, can only see reviews you wrote, 
    # so can only edit those.
    review = Review.query.filter_by(review_id=review_id).first()
    # return str(review.review_body)
    # if not user_id == review.user_id:
    #     return make_response("You can only delete your own post", 403)
    if not review:
        return make_response(f"Review {review_id} does not exist.", 404)
    review.delete()
    flash(f"Review {review_id} has been deleted", 'warning')
    # return make_response(f"Review {review_id} has been deleted", 200)
    movie_id = review.movie_id
    primary_movie_data = (requests.get(f'https://api.themoviedb.org/3/movie/{movie_id}?api_key={TMDB_API_KEY}')).json()
    poster_path_suffix = primary_movie_data["poster_path"]
    poster_url=f"https://image.tmdb.org/t/p/w92{poster_path_suffix}"
